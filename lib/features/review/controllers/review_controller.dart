...
import 'package:flutter_sixvalley_ecommerce/features/review/domain/models/review_response_model.dart';
...
class ReviewController extends ChangeNotifier {
  ...
  ReviewResponseModel? _reviewResponse;
  int _currentPage = 1;
  int _totalReviews = 0;
  String? _productSlug;
  ...
  String? get averageRating => _reviewResponse?.averageRating;

  int getStarCount(int star) {
    return _reviewResponse?.groupWiseRating?.firstWhere((g) => g.rating == star, orElse: () => GroupWiseRating()).total ?? 0;
  }


  Future<void> getReviewList(int offset, {String? productSlug}) async {
    if (productSlug != null) {
    ...
    ApiResponseModel reviewResponse = await reviewServiceInterface.get(_productSlug.toString(), offset: offset, limit: 10,);

    if (reviewResponse.response != null && reviewResponse.response!.statusCode == 200) {

      final parsed = ReviewResponseModel.fromJson(reviewResponse.response!.data);

      _reviewResponse = parsed;
      _totalReviews = parsed.totalSize ?? 0;
      _currentPage = offset;

      if (offset == 1) {
        _reviewList = parsed.reviews ?? [];
      } else {
        _reviewList = [...?_reviewList, ...?parsed.reviews];
      }

      _reviewList!.sort((a, b) => DateTime.parse(b.createdAt!).compareTo(DateTime.parse(a.createdAt!)));
